        ...
        ZStack(alignment: mainZStackAlignment) {
            ForEach((0..<buttons.count), id: \.self) { idx in
                buttons[idx]
                    ...
                    .offset(alignmentOffsets.isEmpty ? .zero : alignmentOffsets[idx])
                    .offset(buttonOffset(at: idx))
                    .scaleEffect(isOpen ? 1 : initialScaling)
                    .opacity(mainButtonSize == .zero ? 0 : isOpen ? 1 : initialOpacity)
                    .animation(buttonAnimation(at: idx), value: isOpen)
                    .zIndex(Double(inverseZIndex ? (buttons.count - idx - 1) : 0))
            }
